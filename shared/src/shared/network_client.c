...
static bool too_much_data(network_client_t *client,
			...
			size_t length)
{
	if (client->bstate == FREED) {
		...
		client->buffer = realloc
			(client->buffer,
			sizeof(char) * (client->length + length));
	...
	&& too_much_data(client, client->length, max_size)) goto done;
	...
	for (size_t i = 0; i < len - data_remaining; ++i) {
		...
		if (!too_much_data(client, packet_length, max_size)) {
			server->client_handler.on_received
				(client, packet, packet_length);
