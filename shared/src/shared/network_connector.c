...
static network_client_t *hold_new_client(network_instance_t *instance,
					session_t id) {
...
static void read_data(network_instance_t *instance, int client) {
	...
	network_client_t *found = network_client_find(&instance->clients,
						client);
...
static void on_client_ready(session_t client, void *server_ptr) {
	...
	if (client == ERROR) {
		for (iter_t *it = iter_begin(&server->clients); it;
		iter_next(it)) {
